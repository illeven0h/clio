...
import RecommendationScroller from "../../components/RecommendationScroller";
...
export default function Spotlight() {
  ...
  return (
    <>
      <div className="pb-8">
        <h1 className="text-2xl font-bold mb-6 text-grey">Recommendations</h1>
        <RecommendationScroller />
      </div>

      <div className="px-6 min-h-screen">
        <h1 className="text-2xl font-bold mb-6 text-grey">Trending Videos</h1>
        ...
        {videos.length === 0 ? (
          <div className="bg-background rounded-lg shadow-md p-6 text-center">
            ...
            <p className="text-xl text-gray-600 mt-2">No videos found in the database.</p>
            <p className="text-gray-500 mt-2">Videos uploaded by users will appear here.</p>
          </div>
        ) : (
            ...
            {videos.map((video) => (
              <div key={video.id} className="bg-background rounded-lg shadow-md overflow-hidden flex flex-col h-full transform transition-transform hover:scale-[1.02] hover:shadow-lg">
                ...
                <div className="p-4 flex-grow flex flex-col">
                  <h3 className="text-lg font-semibold text-gray-800 mb-2 line-clamp-2">{video.title || "Untitled Video"}</h3>
                  <p className="text-sm text-gray-500 truncate">By: {video.uploadedBy || "Unknown"}</p>
                  <p className="text-xs text-gray-500 mt-1">{video.uploadedAt ? new Date(video.uploadedAt.seconds * 1000).toLocaleDateString() : ""}</p>
                </div>
              </div>
            ))}
          </div>
